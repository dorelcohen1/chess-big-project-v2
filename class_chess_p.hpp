...
// enum for all the Result of a move and the code we send to the front end
enum MoveResult 
{
    Valid = 0,                                     // the result is a valid move code 0
    Valid_Check = 1,                               // the result is a valid move that is Check code 1
    Invalid_NoPieceAtSource = 2,                   // the result is a Invalid move with No Piece At Source point code 2
    Invalid_DestinationOccupiedByOwnPiece = 3,     // the result is a Invalid move with Destination Occupied By Own Piece code 3
    Invalid_SelfCheck = 4,                         // the result is a Invalid move that results in Self Check code 4
    Invalid_IndexOutOfRange = 5,                   // the result is a Invalid move with Index Out Of Range code 5
    Invalid_IllegalMovement = 6,                   // the result is a Invalid move that is Illegal by Movement code 6
    Invalid_SameSourceAndDestination = 7,          // the result is a Invalid move that have the Same Source And Destination code 7
    Valid_Checkmate = 8                            // the result is a valid move and Checkmate code 8
};
