...
const std::string STARTING_BOARD_STATE = "RNBQKBNRPPPPPPPP################################pppppppprnbqkbnr0"; // Initial board state
...
public:
    ...
    MoveResult get_result() const;

    // --- setters --- //

    void set_try_move(const std::string& move);

    // --- Other Methods --- //

    MoveResult Move();
    bool kill_at(int row, int col);
